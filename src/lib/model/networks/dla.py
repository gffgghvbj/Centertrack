...
class DLA(nn.Module):
    ...
    def load_pretrained_model(self, data='imagenet', name='dla34', hash='ba72cf86'):
        ...
        if name.endswith('.pth'):
            ...
            model_weights = load_url(model_url, progress=True)

        # 使用tqdm显示加载进度
        with tqdm(total=len(model_weights), unit="item") as pbar:
        ...
        # 使用tqdm显示加载状态字典的进度
        with tqdm(total=len(model_weights), unit="item", desc="Loading weights") as pbar:
            self.load_state_dict(model_weights, strict=False)
            pbar.update(len(model_weights))
        # self.fc = fc
